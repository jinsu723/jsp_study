...
for (let i = 1; i <= postCount.postNumber; i++) { //게시글 총합 가져오기(페이지수에 따라 하단 숫자 바뀜)
  /*posts.push({
    writer: postList.userNickname,
    category: `공략`,
    title: `게시물 제목 ${i}`,
    writeTime: `${year}-${month}-${day} ${hours}:${minutes}`,
    postId: `게시글${postList.postNumber}`
  });*/
}
...
function getPost(page) {
  ...
  postList.forEach(post => {
	const DateTimeFormat = new Date(post.postData);
    ...
    postItem.innerHTML = `
      ...
      <div class="communityForum-list-category">${post.postCategory}</div>
      <div class="communityForum-list-title">${post.postTitle}</div>
    ...
    postItem.addEventListener('click', () => {
		const postDetails = `${contextPath}/app/communityForum/communityForumDetail.cf?postNum=${post.postNumber}`;
		...
		fetch(`http://localhost:8888${contextPath}/app/communityForum/communityForumDetail.jsp?postNum=${post.postNumber}`, {
		  ...
		  body: JSON.stringify(`${post.postNumber}`)
		});
