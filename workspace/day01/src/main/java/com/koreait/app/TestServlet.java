...
//		response.getWriter().append("Serverd at: ").append(request.getContextPath());
		...
		// 응답하는 컨텐츠가 html이고 문자 인코딩은 UTF-8을 사용할거라는 설정을 해준다
...
//		out.print("<html>");  // out.print() : PrintWriter 객체를 사용하여 HTML 코드를 생성하고 브라우저로 전송
//		out.print("<head>");
...
//		getParameter(String atgs) 꺼내고 싶은 뎅터의 이름을 넘겨주면 된다
//		데이터의 이름은 html에 작성한 태그의 name 태그의 값과 동일하다
		String result = request.getParameter("userName");
		System.out.println(result);
		
		// ContentType은 현제 페이지의 컨텐츠들을 의미한다, 다른 의미로는 현테 페이지에 뿌려지는 데이터들의 정보를 담고있다
		// 응답을 할 때도 올바르게 설정되어 있어야 브라우저가 정확하게 해석하고 화면을 보여줄 수 있다
//		response.setContentType("text/html; charset=UTF-8");
		
		// response 객체는 getWriter()를 가지고 있다
		// getWriter()의 반환 타입은 PrintWriter이다
		// 즉 PrintWriter 객체를 반환하며 이 객체는 스트림을 사용하는 출력 객체이다
		// 이 객체에 print()라는 메소드를 사용하면 클라이언트 화면에 보여줄 문자열을 응답객체에 담을 수 있다
//		PrintWriter out = response.getWriter();
...
//		out.close();
		// 위의 코드도 여전히 서블릿 방식을 사용하고 있으므로 결과 페이지를 만들 result.jsp 파일을 만들고 jsp 파일로 실행시킬 것이다
		
		// forward 방식 사용
		// getRequestDispatcher("경로").forward(request, response)
		// 다른 서블릿이나 jsp로 요청을 전달한다
		// 주로 클라이언트의 요청을 받아서 다른 페이지로 이동을 시킬 때 사용한다
		// getRequestDispatcher는 request 객체가 가지고 있으며 request 객체를 전달할 파일 경로를 매개변수로 넘겨줘야 한다
		// forward()에 request 객체를 넘겨주므로 request 객체가 가지고 있는 데이터를 유지한 채로 jsp 파일로 이동한다
